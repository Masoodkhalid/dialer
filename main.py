...
from fastapi import Body, Depends, FastAPI, HTTPException, Query, Request, UploadFile, WebSocket, WebSocketDisconnect
...
async def admin_assign_subscription(body: dict, payload: dict = Depends(require_admin)):
    """Admin: assign or replace a subscription for a user."""
    username = (body.get("username") or "").strip().lower()
    if not username or username not in users:
        raise HTTPException(404, "User not found")

    did_id = body.get("did_id") or ""
    did = next((d for d in dids if d.id == did_id), None) if did_id else None

    # Free old DID if any
    old_sub = subscriptions.get(username)
    if old_sub:
        old_did = next((d for d in dids if d.id == (old_sub.did_id or "")), None)
        if old_did:
            old_did.owner_username = None

    if did:
        did.owner_username = username
        plan_name  = body.get("plan_name") or f"USA {did.minutes}-min Pack"
        price      = float(body.get("price", did.price))
        minutes    = int(body.get("minutes", did.minutes))
        did_number = did.number
    else:
        plan_name  = body.get("plan_name") or "Custom Plan"
        price      = float(body.get("price", 5.0))
        minutes    = int(body.get("minutes", 10))
        did_number = body.get("did_number") or ""
    ...
    sub = Subscription(
        ...
        minutes_used=0.0,
        is_active=True,
        app_id=getattr(users.get(username), "app_id", None),
    )
    subscriptions[username] = sub
    _save()
...
@app.patch("/admin/subscriptions/{username}")
async def admin_update_subscription(username: str, body: dict, payload: dict = Depends(require_admin)):
    """Admin: edit a user's subscription (plan name, price, minutes, status)."""
    sub = subscriptions.get(username)
    if not sub:
        raise HTTPException(404, "No subscription for this user")
    if "plan_name"    in body: sub.plan_name    = body["plan_name"]
    if "price"        in body: sub.price        = float(body["price"])
    if "minutes_total" in body: sub.minutes_total = int(body["minutes_total"])
    if "minutes_used" in body: sub.minutes_used  = float(body["minutes_used"])
    if "is_active"    in body: sub.is_active    = bool(body["is_active"])
    if "add_minutes"  in body: sub.minutes_total += int(body["add_minutes"])
    _save()
    return {"status": "ok"}


@app.delete("/admin/subscriptions/{username}")
async def admin_delete_subscription(username: str, payload: dict = Depends(require_admin)):
    """Admin: cancel and remove a user's subscription, freeing their DID."""
    sub = subscriptions.pop(username, None)
    if not sub:
        raise HTTPException(404, "No subscription found")
    did = next((d for d in dids if d.id == (sub.did_id or "")), None)
    if did:
        did.owner_username = None
    _save()
    return {"status": "deleted"}
...
@app.patch("/admin/apps/{app_id}/rename")
async def rename_app(app_id: str, body: dict, payload: dict = Depends(require_admin)):
    """Admin: rename an app — updates app_id on all users and subscriptions."""
    new_id = (body.get("new_name") or "").strip()
    if not new_id:
        raise HTTPException(400, "new_name required")
    count = 0
    for u in users.values():
        if (getattr(u, "app_id", None) or "default") == app_id:
            u.app_id = new_id; count += 1
    for s in subscriptions.values():
        if (getattr(s, "app_id", None) or "default") == app_id:
            s.app_id = new_id
    _save()
    return {"status": "ok", "updated_users": count, "new_app_id": new_id}


@app.delete("/admin/apps/{app_id}")
async def delete_app(app_id: str, payload: dict = Depends(require_admin)):
    """Admin: clear an app — sets all users' app_id to null."""
    count = 0
    for u in users.values():
        if (getattr(u, "app_id", None) or "default") == app_id:
            u.app_id = None; count += 1
    for s in subscriptions.values():
        if (getattr(s, "app_id", None) or "default") == app_id:
            s.app_id = None
    _save()
